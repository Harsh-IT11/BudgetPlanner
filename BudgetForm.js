...
export default function BudgetForm(props) {
  ...
  useEffect(()=>{
    const fetchData = async()=>{
      try {
        const response = await axios.get(`${backendUrl}/budget-items/get-total-income`)
  ...
  useEffect(()=>{
    const fetchData = async()=>{
      try {
        const response = await axios.get(`${backendUrl}/budget-items/total-amount`)
  ...
  return (
    <>
      <div className="container card my-4 rounded shadow latoFont budgetform  ">
        <div  style={{ display: "flex", alignItems: "center",padding : '20px',overflow:'hidden'}}>
         ...
         <ChangingProgressProvider values={[0,percent]}>
          ...
          <p className="montFont" style={{fontSize:'25px',margin : '0px'}}>Total remaining budget</p>
            <div>
              <span style={{ fontSize: '45px',color:'#4F4557' }}>&#8377;{categTotal}</span>
              <span className="text-muted" style={{ fontSize: '25px' }}>/&#8377;{income}</span>   
